export const zh = {
  nav: {
    signIn: '登录',
    signUp: '免费开始',
    goToDashboard: '进入控制台',
    logout: '退出登录',
  },
  home: {
    badge: 'AI 驱动的产品演示生成器',
    headline1: '粘贴 URL，',
    headline2: '秒生成演示视频',
    sub: '输入产品网址，AI 解析操作流程、自动录制、合成旁白，\n生成专业的可分享演示视频。',
    ctaStart: '免费开始使用',
    ctaSignIn: '已有账号登录',
    ctaNote: '免费生成 3 个 Demo · 无需信用卡',
    stats: [
      { value: '< 5min', label: '平均生成时长' },
      { value: '100%', label: '全自动化录制' },
      { value: '无限制', label: '分享链接永久有效' },
    ] as { value: string; label: string }[],
    featuresTitle: '全自动化演示生成',
    featuresSub: '三步生成专业演示视频',
    features: [
      { title: 'AI 智能解析', desc: 'DeepSeek 读取页面结构，自动规划完整操作步骤' },
      { title: '自动录制', desc: 'Playwright 驱动真实浏览器，逐步录制演示流程' },
      { title: '一键分享', desc: '生成专属分享链接，带章节导航的视频播放页' },
    ] as { title: string; desc: string }[],
    ctaTitle: '开始生成你的第一个 Demo',
    ctaSub: '免费注册，3 个 Demo 无需信用卡',
    ctaBtn: '立即免费开始',
    footer: '© 2025 Showrunner · All rights reserved',
  },
  dashboard: {
    title: 'Demo 工作台',
    subtitle: '粘贴产品 URL，AI 自动生成可分享的演示视频',
    demosCount: (n: number) => `${n} 个 Demo`,
    myDemos: '我的 Demo',
    noDemo: '还没有 Demo',
    noDemoSub: '在上方输入产品 URL 开始生成第一个演示视频',
  },
  createForm: {
    title: '生成新的 Demo',
    urlLabel: '产品 URL',
    urlPlaceholder: 'https://app.yourproduct.com',
    descLabel: '演示内容',
    descOptional: '（可选）',
    descPlaceholder: '例：用户注册 → 创建项目 → 导出报告',
    hint: 'AI 将自动解析页面并规划操作步骤',
    submitBtn: '生成 Demo',
    loadingBtn: 'AI 解析中...',
    errorDefault: '创建失败，请重试',
    errorNetwork: '网络错误，请重试',
  },
  demoCard: {
    view: '查看',
    reviewSteps: '确认步骤',
    handle: '处理',
  },
  status: {
    pending: '待处理',
    parsing: 'AI 解析中',
    review: '待确认',
    recording: '录制中',
    paused: '已中断',
    processing: '合成中',
    completed: '已完成',
    failed: '失败',
  },
  auth: {
    signIn: {
      title: '欢迎回来',
      subtitle: '登录你的 Showrunner 账号',
      google: '使用 Google 账号登录',
      github: '使用 GitHub 账号登录',
      orEmail: '或使用邮箱登录',
      emailLabel: '邮箱地址',
      passLabel: '密码',
      passPh: '••••••••',
      submitBtn: '登录',
      loadingBtn: '登录中...',
      errorDefault: '登录失败',
      errorNetwork: '网络错误，请重试',
      noAccount: '没有账号？',
      signUpLink: '免费注册',
    },
    signUp: {
      title: '创建账号',
      subtitle: '免费开始 · 无需信用卡',
      google: '使用 Google 账号注册',
      github: '使用 GitHub 账号注册',
      orEmail: '或使用邮箱注册',
      emailLabel: '邮箱地址',
      passLabel: '密码',
      passPh: '至少 8 位',
      submitBtn: '创建账号',
      loadingBtn: '注册中...',
      terms: '注册即同意 Showrunner 使用条款',
      hasAccount: '已有账号？',
      signInLink: '登录',
      errorDefault: '注册失败',
      errorNetwork: '网络错误，请重试',
    },
    oauthErrors: {
      oauth_denied: '授权被取消，请重试',
      oauth_state_mismatch: '安全验证失败，请重试',
      oauth_token_failed: 'OAuth 令牌获取失败，请重试',
      oauth_profile_failed: '获取账号信息失败，请重试',
      oauth_no_email: '无法获取邮箱地址，请确保已授权邮箱访问',
    },
  },
}
